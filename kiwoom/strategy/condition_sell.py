...
class ConditionSell(StrategyBase):
    ...
    def on_condition(self, condition_index, condition_name):
        MyLogger.instance().logger().info("index: %d, name: %s, 종목명: %s", condition_index, condition_name, self.balance.종목명)
        if self.is_queued:
        ...
        MyLogger.instance().logger().info("종목명: %s, 수익률: %f, 현재가: %d, 매입가: %d, 보유수량: %d", self.balance.종목명, 수익률, self.balance.현재가, self.balance.매입가, self.balance.보유수량)
